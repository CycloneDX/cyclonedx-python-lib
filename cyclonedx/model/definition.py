...
from ..exception.model import CycloneDxModelException
...
class CreId(serializable.helpers.BaseHelper):
    ...
    def deserialize(cls, o: Any) -> 'CreId':
        try:
            return CreId(id=str(o))
        except ValueError as err:
            raise CycloneDxDeserializationException(
                f'CreId string supplied does not parse: {o!r}'
            ) from err
...
class Requirement:
    ...
    def __init__(
        ...
        self._parent = bom_ref_or_str(parent)
...
class Level:
    ...
    def requirements(self, requirements: Iterable[Union[str, BomRef]]) -> None:
        self._requirements = SortedSet(map(bom_ref_or_str, requirements))
